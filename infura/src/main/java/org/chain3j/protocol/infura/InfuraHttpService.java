...
public class InfuraHttpService extends HttpService {
    ...
    private static final String INFURA_moac_PREFERRED_CLIENT =
    ...
    static Map<String, String> buildClientVersionHeader(String clientVersion, boolean required) {
        ...
        if (required) {
            return Collections.singletonMap(INFURA_moac_PREFERRED_CLIENT, clientVersion);
        } else {
            return Collections.singletonMap(
                    INFURA_moac_PREFERRED_CLIENT, clientVersion + "; required=false");
        }
    }
}
